...
class TrainData:
    ...
    def get_scheduled_train_details (self, date = '', startlocation = '', endlocation = ''):
        ...
        if startlocation == '' and endlocation == '' and date == '':
            ...
            query = """select * from tblTrainSchedules natural join tblTrains where start_location = %s"""
            parameters = (startlocation, )
            self.cursorObject.execute(query, parameters)
            data = self.cursorObject.fetchall()
        else:
            query = """select * from tblTrainSchedules natural join tblTrains where start_location = %s and end_location = %s"""
